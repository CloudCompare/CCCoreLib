...
namespace CCCoreLib
{
	...
	public:
		...
		**/
		...
		inline Scalar* row( unsigned index )
		{
		...
		**/
		bool toQuaternion( double q[/*4*/] )
		{
			...
			double dTrace = static_cast<double>( m_values[0][0] ) + static_cast<double>( m_values[1][1] ) +
		...
		double computeSubDet( Scalar** mat, unsigned matSize ) const
		{
			...
			if ( subMat )
			{
				...
				for ( unsigned row = 0; row < matSize; row++ )
				{
					...
					subDet +=
						static_cast<double>( mat[row][0] ) * computeSubDet( subMat, matSize - 1 ) * sign;
					sign = -sign;
				}
