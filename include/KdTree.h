...
namespace CCCoreLib
{
	...
	protected:
		...
		struct KdCell
		{
			...
			/** The outside bounding box is the bigest cube contained inside the cutting planes that lead to
			  *the cell
			 **/
			...
			/** if boundsMask & (2^d) then outbbmin.u[d] is bounded (else the box is opened in outmin.u[d] -
			i.e. outbbmin.u[d] = -infinite) if boundsmask & (2^(3+d)) then outbbmax.u[d] is bounded (else the
			box is opened in outmax.u[d] - i.e. outbbmax.u[d] = infinite)
			**/
		...
		**/
		...
		//! Computes a cell inside bounding box using the sons ones. The sons bounding boxes have to be up to
		//! date considering the points they contain.
		...
		**/
		...
		//! Checks if there is a point in KdCell that is less than minDist-apart from the query point,
		//! starting from cell cell
		/** Optimiszed version of CheckNearerPointInSubTree since we don't want to find the nearest point, but
		only check if there is a point that is close enough
			\param queryPoint the query Point coordinates
			\param maxSqrDist square of the maximal distance from querypoint
			\param cell kdtree-cell from which to start the research
