	...
	public:
		...
		void setPointScalarValue( unsigned pointIndex, ScalarType value ) override
		{
			assert( m_currentInScalarFieldIndex >= 0 &&
					m_currentInScalarFieldIndex < static_cast<int>( m_scalarFields.size() ) );
			// slow version
			// ScalarField* currentInScalarFieldArray = getCurrentInScalarField();
			// if (currentInScalarFieldArray)
		...
		ScalarType getPointScalarValue( unsigned pointIndex ) const override
		{
			assert( m_currentOutScalarFieldIndex >= 0 &&
					m_currentOutScalarFieldIndex < static_cast<int>( m_scalarFields.size() ) );
		...
		**/
		...
		{
			return ( index >= 0 && index < static_cast<int>( m_scalarFields.size() ) ? m_scalarFields[index]
																					 : 0 );
		}
		...
		**/
		virtual int addScalarField( const char* uniqueName )
		{
			...
			try
			{
				// we don't want 'm_scalarFields' to grow by 50% each time! (default behavior of
