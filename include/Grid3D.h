...
namespace CCCoreLib
{
	...
	public:
		...
		inline const Tuple3ui& size() const
		{
			return m_innerSize;
		}
		...
		**/
					 ...
					 GreaterThanEpsilon( CA.norm2() ) )
				{
					Tuple3i cellPos[3];
					{
						for ( int k = 0; k < 3; k++ )
						{
							...
							cellPos[k].x = std::min( static_cast<int>( P.x / cellLength ),
													 static_cast<int>( size().x ) - 1 );
							cellPos[k].y = std::min( static_cast<int>( P.y / cellLength ),
													 static_cast<int>( size().y ) - 1 );
							cellPos[k].z = std::min( static_cast<int>( P.z / cellLength ),
													 static_cast<int>( size().z ) - 1 );
						}
					}
					...
					Tuple3i minPos, maxPos;
					{
						...
						{
							minPos.u[k] =
								std::min( cellPos[0].u[k], std::min( cellPos[1].u[k], cellPos[2].u[k] ) );
							maxPos.u[k] =
								std::max( cellPos[0].u[k], std::max( cellPos[1].u[k], cellPos[2].u[k] ) );
						}
					}
					...
					_currentCell->cellSize =
						( 1 << ( maxSize > 1 ? static_cast<unsigned char>(
												   ceil( log( static_cast<double>( maxSize ) ) / LOG_2 ) )
											 : 0 ) );
					...
					while ( cellsToTestCount != 0 )
					{
						...
						if ( _currentCell->cellSize == 1 )
						{
							...
							AB = gridMinCorner + CCVector3::fromArray( currentCellPos.u ) * cellLength +
								 halfCellDimensions;
							...
							if ( CCMiscTools::TriBoxOverlap( AB, halfCellDimensions, triPoints ) )
							{
								if ( ( currentCellPos.x >= 0 &&
									   currentCellPos.x < static_cast<int>( size().x ) ) &&
									 ( currentCellPos.y >= 0 &&
									   currentCellPos.y < static_cast<int>( size().y ) ) &&
									 ( currentCellPos.z >= 0 &&
									   currentCellPos.z < static_cast<int>( size().z ) ) )
								{
						...
						else
						{
							...
							char pointsPosition[27];
							{
								...
								for ( int i = 0; i < 3; ++i )
								{
									AB.x = distanceToMinBorder.x + static_cast<PointCoordinateType>(
																	   currentCellPos.x + i * halfCellSize ) *
																	   cellLength;
									for ( int j = 0; j < 3; ++j )
									{
										AB.y = distanceToMinBorder.y +
											   static_cast<PointCoordinateType>( currentCellPos.y +
																				 j * halfCellSize ) *
												   cellLength;
										for ( int k = 0; k < 3; ++k )
										{
									...
									for ( int j = 0; j < 2; ++j )
									{
											 ...
											 static_cast<int>( _newCell->pos.y ) <= maxPos.y )
										{
											for ( int k = 0; k < 2; ++k )
											{
												...
												if ( static_cast<int>( _newCell->pos.z ) + halfCellSize >=
														 minPos.z &&
							 ...
							 GenericProgressCallback* progressCb = nullptr )
		{
			...
			for ( unsigned n = 0; n < numberOfPoints; ++n )
			{
				...
				Tuple3i cellPos(
					std::min( static_cast<int>( P.x / cellLength ), static_cast<int>( size().x ) - 1 ),
	...
	protected:
		...
		inline int pos2index( int i, int j, int k ) const
		{
			return i + j * static_cast<int>( m_rowSize ) + k * static_cast<int>( m_sliceSize ) +
				   static_cast<int>( m_marginShift );
		}
