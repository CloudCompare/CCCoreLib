	...
	public:
		...
		inline const CCVector3* getNextPoint() override
		{
			...
			return ( m_globalIterator < size()
						 ? m_theAssociatedCloud->getPoint( m_theIndexes[m_globalIterator++] )
						 : nullptr );
		}
		...
		inline void setPointScalarValue( unsigned pointIndex, ScalarType value ) override
		{
			assert( m_theAssociatedCloud && pointIndex < size() );
			m_theAssociatedCloud->setPointScalarValue( m_theIndexes[pointIndex], value );
		}
		inline ScalarType getPointScalarValue( unsigned pointIndex ) const override
		{
			assert( m_theAssociatedCloud && pointIndex < size() );
			return m_theAssociatedCloud->getPointScalarValue( m_theIndexes[pointIndex] );
		}
		...
		inline const CCVector3* getPointPersistentPtr( unsigned index ) const override
		{
			assert( m_theAssociatedCloud && index < size() );
		...
		inline virtual ScalarType getCurrentPointScalarValue() const
		{
			...
			return m_theAssociatedCloud->getPointScalarValue( m_theIndexes[m_globalIterator] );
		}
		...
		{
			assert( m_theAssociatedCloud && m_globalIterator < size() );
			m_theAssociatedCloud->setPointScalarValue( m_theIndexes[m_globalIterator], value );
		}

		//! Forwards the local element iterator
		inline virtual void forwardIterator()
		{
			++m_globalIterator;
		}
		...
		**/
		...
		inline virtual const GenericIndexedCloudPersist* getAssociatedCloud() const
		{
			return m_theAssociatedCloud;
		}
