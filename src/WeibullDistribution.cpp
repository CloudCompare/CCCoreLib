...
bool WeibullDistribution::computeParameters( const ScalarContainer& values )
{
	...
	return setParameters( static_cast<ScalarType>( a ),
						  static_cast<ScalarType>( valueRange * pow( b / counter, 1.0 / a ) ), minValue );
}
...
{
	return ( x <= m_valueShift ? 0.0
							   : 1.0 - exp( -pow( static_cast<double>( x - m_valueShift ) / m_b,
...
double WeibullDistribution::computeP( ScalarType x1, ScalarType x2 ) const
{
	...
	return exp( -pow( static_cast<double>( x1 - m_valueShift ) / m_b, static_cast<double>( m_a ) ) ) -
...
double WeibullDistribution::computeSkewness() const
{
	...
	return ( Gamma_cc( 1.0 + 3.0 / m_a ) * ( m_b * m_b * m_b ) - 3.0 * m_mu * m_sigma2 -
			 ( m_mu * m_mu * m_mu ) ) /
