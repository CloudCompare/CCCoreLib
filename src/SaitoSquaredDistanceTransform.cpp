...
bool SaitoSquaredDistanceTransform::SDT_3D( Grid3D<GridElement>& grid,
											GenericProgressCallback* progressCb /*=0*/ )
{
	...
	std::size_t diag =
		static_cast<std::size_t>( ceil( sqrt( static_cast<double>( r * r + c * c + p * p ) ) ) - 1 );
	...
	const GridElement maxDistance =
		std::numeric_limits<GridElement>::max() - static_cast<GridElement>( r * r + c * c + p * p ) - 1;
