...
TrueKdTree::BaseNode* TrueKdTree::split( ReferenceCloud* subset )
{
	...
	if ( count < m_maxPointCountPerCell || count < 2 * m_minPointCountPerCell )
	{
		...
		error = ( count > 3 ? DistanceComputationTools::ComputeCloud2PlaneDistance( subset, planeEquation,
																					m_errorMeasure )
							: 0 );
	...
	if ( s_sortedCoordsForSplit[splitCount - 1] == s_sortedCoordsForSplit[splitCount] )
	{
				  ...
				  s_sortedCoordsForSplit[splitCount] ) // can we go forward?
		{
			...
			} while ( /*splitCount+1<count &&*/ s_sortedCoordsForSplit[splitCount] ==
		...
		else // in fact we can't split this cell!
		{
			UpdateProgress( count );
			if ( error < 0 )
				error = ( count != 3 ? DistanceComputationTools::ComputeCloud2PlaneDistance(
