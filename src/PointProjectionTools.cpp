																...
																std::string& outputErrorStr )
{
	...
	case DELAUNAY_2D_BEST_LS_PLANE:
	{
		...
		GenericIndexedMesh* mesh =
			Yk.triangulateOnPlane( Neighbourhood::DO_NOT_DUPLICATE_VERTICES, maxEdgeLength, outputErrorStr );
												...
												std::list<IndexedCCVector2*>& hullPoints )
{
	...
	if ( hullPoints.size() > 1 && hullPoints.front()->x == hullPoints.back()->x &&
	...
	for ( unsigned i = 0; i < pointCount; ++i )
	{
		...
		if ( dot >= 0 && dot <= squareLengthAB )
		{
			...
			if ( minDist2 < 0 || dist2 < minDist2 )
			{
				...
				if ( squareLengthAP >= minSquareEdgeLength && squareLengthBP >= minSquareEdgeLength &&
					 ( allowLongerChunks ||
					   ( squareLengthAP < squareLengthAB || squareLengthBP < squareLengthAB ) ) )
				{
					minDist2 = dist2;
					minIndex = i;
				}
												 ...
												 PointCoordinateType maxSquareEdgeLength /*=0*/ )
{
	...
	PointCoordinateType minSquareEdgeLength = 0;
	{
		...
		minSquareEdgeLength =
			( maxP - minP ).norm2() /
			static_cast<PointCoordinateType>( 1.0e7 ); // 10^-7 of the max bounding rectangle side
		minSquareEdgeLength = std::min( minSquareEdgeLength, maxSquareEdgeLength / 10 );

		// we remove very small edges
		for ( std::list<IndexedCCVector2*>::iterator itA = hullPoints.begin(); itA != hullPoints.end();
			  ++itA )
		{
	...
	while ( somethingHasChanged )
	{
		try
		{
					  ...
					  itA != hullPoints.end(); ++itA )
				{
					...
					if ( ( **itB - **itA ).norm2() > maxSquareEdgeLength )
					{
						...
						PointCoordinateType minSquareDist =
							FindNearestCandidate( nearestPointIndex, itA, itB, points, pointFlags,
												  minSquareEdgeLength, maxSquareEdgeLength, step > 1 );
			...
			while ( !edges.empty() )
			{
				...
				// if (false)
				{
					for ( VertexIterator itJ = hullPoints.begin(), itI = itJ++; itI != hullPoints.end();
						  ++itI, ++itJ )
					{
						if ( itJ == hullPoints.end() )
							itJ = hullPoints.begin();

						if ( ( ( *itI )->index != ( *itA )->index && ( *itJ )->index != ( *itA )->index &&
							   PointProjectionTools::segmentIntersect( **itI, **itJ, **itA, P ) ) ||
							 ( ( *itI )->index != ( *itB )->index && ( *itJ )->index != ( *itB )->index &&
							   PointProjectionTools::segmentIntersect( **itI, **itJ, P, **itB ) ) )
						{
							intersect = true;
							break;
						}
				...
				if ( !intersect )
				{
					...
					VertexIterator itP = hullPoints.insert(
						itB == hullPoints.begin() ? hullPoints.end() : itB, &points[e.nearestPointIndex] );
					...
					if ( !edges.empty() )
					{
						...
						for ( std::multiset<Edge>::const_iterator it = edges.begin(); it != edges.end();
							  ++it )
						{
						...
						for ( std::size_t i = 0; i < removed.size(); ++i )
						{
							...
							unsigned nearestPointIndex = 0;
							PointCoordinateType minSquareDist =
								FindNearestCandidate( nearestPointIndex, itC, itD, points, pointFlags,
													  minSquareEdgeLength, maxSquareEdgeLength );
					...
					if ( ( P - **itA ).norm2() > maxSquareEdgeLength )
					{
						unsigned nearestPointIndex = 0;
						PointCoordinateType minSquareDist =
							FindNearestCandidate( nearestPointIndex, itA, itP, points, pointFlags,
												  minSquareEdgeLength, maxSquareEdgeLength );
					...
					if ( ( **itB - P ).norm2() > maxSquareEdgeLength )
					{
						unsigned nearestPointIndex = 0;
						PointCoordinateType minSquareDist =
							FindNearestCandidate( nearestPointIndex, itP, itB, points, pointFlags,
												  minSquareEdgeLength, maxSquareEdgeLength );
