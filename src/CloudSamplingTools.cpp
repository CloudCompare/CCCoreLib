																  ...
																  DgmOctree* inputOctree )
{
	...
	GenericIndexedCloud* sampledCloud =
		resampleCloudWithOctreeAtLevel( inputCloud, bestLevel, resamplingMethod, progressCb, octree );
																...
																DgmOctree* inputOctree /*=0*/ )
{
	...
	if ( octree->executeFunctionForAllCellsAtLevel( octreeLevel, &resampleCellAtLevel, additionalParameters,
															  ...
															  DgmOctree* inputOctree /*=0*/ )
{
	...
	ReferenceCloud* subsampledCloud =
		subsampleCloudWithOctreeAtLevel( inputCloud, bestLevel, subsamplingMethod, progressCb, octree );
	...
	DgmOctree* inputOctree /*=0*/ )
{
	...
	if ( octree->executeFunctionForAllCellsAtLevel( octreeLevel, &subsampleCellAtLevel, additionalParameters,
													false, // the process is so simple that MT is slower!
	...
	try
	{
		if ( modParams.enabled )
		{
			...
			else
			{
				...
				PointCoordinateType dist0 =
					static_cast<PointCoordinateType>( sfMin * modParams.a + modParams.b );
				PointCoordinateType dist1 =
					static_cast<PointCoordinateType>( sfMax * modParams.a + modParams.b );
				...
				if ( level1 != level0 )
				{
					...
					for ( std::size_t i = 1; i < levelCount - 1; ++i ) // we already know level0 and level1!
					{
						...
						PointCoordinateType dist =
							static_cast<PointCoordinateType>( sfVal * modParams.a + modParams.b );
						unsigned char level =
							octree->findBestLevelForAGivenNeighbourhoodSizeExtraction( dist );
		...
		else
		{
			unsigned char defaultLevel =
				octree->findBestLevelForAGivenNeighbourhoodSizeExtraction( minDistance );
	...
	for ( unsigned i = 0; i < cloudSize; i++ )
	{
		...
		if ( markers[i] != 0 )
		{
			...
			// look for neighbors and 'de-mark' them
			{
				...
				octree->getPointsInSphericalNeighbourhood( *P, minDistBetweenPoints, neighbours,
														   octreeLevel );
				for ( DgmOctree::NeighboursSet::iterator it = neighbours.begin(); it != neighbours.end();
					  ++it )
					if ( it->pointIndex != i )
			...
			if ( sampledCloud->size() == sampledCloud->capacity() &&
				 !sampledCloud->reserve( sampledCloud->capacity() + c_reserveStep ) )
			{
				// not enough memory
				error = true;
				break;
											   ...
											   GenericProgressCallback* progressCb /*=0*/ )
{
	...
	for ( unsigned step = 0; step < 1; ++step ) // fake loop for easy break
	{
		...
		// 1st step: compute the average distance to the neighbors
		{
			...
			if ( octree->executeFunctionForAllCellsAtLevel( octreeLevel, &applySORFilterAtLevel,
												 ...
												 GenericProgressCallback* progressCb /*=0*/ )
{
	...
	void* additionalParameters[] = {
		reinterpret_cast<void*>( filteredCloud ),	  reinterpret_cast<void*>( &kernelRadius ),
		reinterpret_cast<void*>( &nSigma ),			  reinterpret_cast<void*>( &removeIsolatedPoints ),
		reinterpret_cast<void*>( &useKnn ),			  reinterpret_cast<void*>( &knn ),
		reinterpret_cast<void*>( &useAbsoluteError ), reinterpret_cast<void*>( &absoluteError )
	};
	...
	if ( octree->executeFunctionForAllCellsAtLevel( octreeLevel, &applyNoiseFilterAtLevel,
													additionalParameters, true, progressCb,
													"Noise filter" ) == 0 )
	{
		// something went wrong
												  ...
												  NormalizedProgress* nProgress /*=0*/ )
{
	...
	for ( unsigned i = 0; i < n; ++i )
	{
		...
		if ( useKnn )
			...
			neighborCount =
				cell.parentOctree->findNeighborsInASphereStartingFromCell( nNSS, kernelRadius, false );
		...
		if ( neighborCount > 3 ) // we want 3 points or more (other than the point itself!)
		{
			...
			while ( localIndex < neighborCount &&
					nNSS.pointsInNeighbourhood[localIndex].pointIndex != globalIndex )
				++localIndex;
			...
			DgmOctreeReferenceCloud neighboursCloud(
				...
				realNeighborCount ); // we don't take the query point into account!
			...
			if ( lsPlane )
			{
				...
				if ( !useAbsoluteError )
				{
					...
					double stddev =
						sqrt( std::abs( sum_d2 * realNeighborCount - sum_d * sum_d ) ) / realNeighborCount;
					maxD = stddev * nSigma;
				}

				// distance from the query point to the plane
				double d = std::abs(
					DistanceComputationTools::computePoint2PlaneDistance( &nNSS.queryPoint, lsPlane ) );
												...
												NormalizedProgress* nProgress /*=0*/ )
{
	...
	std::vector<PointCoordinateType>& meanDistances =
		*static_cast<std::vector<PointCoordinateType>*>( additionalParameters[1] );

	// structure for nearest neighbors search
	DgmOctree::NearestNeighboursSphericalSearchStruct nNSS;
	nNSS.level = cell.level;
	nNSS.minNumberOfNeighbors = knn; // DGM: I woud have put knn+1 (as the point itself will be ignored) but
									 // in this case we won't get the same result as PCL!
	cell.parentOctree->getCellPos( cell.truncatedCode, cell.level, nNSS.cellPos, true );
	cell.parentOctree->computeCellCenter( nNSS.cellPos, cell.level, nNSS.cellCenter );
