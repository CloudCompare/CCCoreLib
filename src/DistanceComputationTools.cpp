...
static void CompareGlobalPointsAndTriangles(	ReferenceCloud& Yk,
...
//! Method used by computeCloud2MeshDistancesWithOctree
static void CompareLocalPointsAndTriangles(	ReferenceCloud& Yk,
	...
	while (trianglesToTestCount != 0)
	{
		...
		if (params.signedDistances)
		{
			...
			for (unsigned j = 0; j < remainingPoints; ++j)
			{
				...
				ScalarType signedDistToTri = DistanceComputationTools::computePoint2TriangleDistance(localPointInMeshCS, localTri, true, _nearestPoint);
				//keep it if it's smaller
				ScalarType min_d = Yk.getPointScalarValue(j);
				if (!ScalarField::ValidValue(min_d) || min_d * min_d > signedDistToTri*signedDistToTri)
				{
...
{
	static ScalarType ConstCellDiag = static_cast<ScalarType>((sqrt(2.0) - 1.0) / 2);
	return static_cast<int>(ceil(maxSearchDist / cellSize + ConstCellDiag));
}
...
static CCVector3 s_localMeshToLocalCloud_MT(0, 0, 0);
...
static void CloudMeshDistCellFunc_MT(const DgmOctree::IndexAndCode& desc)
{
	...
	if (!s_octree_MT || !s_intersection_MT)
	{
		assert(false);
		return;
	}
	...
	s_octree_MT->computeLocalCellCenter(cellPos, s_params_MT.octreeLevel, localCellCenter);
	...
	for (unsigned j = 0; j < remainingPoints; ++j)
	{
		...
		//distance to the closest cell border
	...
	for (int dist = minDistToBoundaries; remainingPoints != 0 && dist <= maxDistToBoundaries; ++dist, maxRadius += static_cast<ScalarType>(cellLength))
	{
		...
		CompareLocalPointsAndTriangles(	Yk,
										s_localMeshToLocalCloud_MT,
										remainingPoints,
										s_intersection_MT->mesh(),
										trianglesToTest,
																	...
																	GenericProgressCallback* progressCb/*=nullptr*/)
{
	...
	else
	{
		...
		s_params_MT = params;
		s_localMeshToLocalCloud_MT = localMeshToLocalCloud;
																	...
																	CCVector3* nearestP/*=nullptr*/)
{
	...
	CCVector3d Q = ProjectPointOnTriangle(AP, AB, AC);
