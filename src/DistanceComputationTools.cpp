														  ...
														  DgmOctree* refOctree /*=0*/ )
{
	...
	SOReturnCode soCode = synchronizeOctrees( comparedCloud, referenceCloud, comparedOctree, referenceOctree,
	...
	double maxSearchSquareDistd =
		params.maxSearchDist <= 0 ? 0 : static_cast<double>( params.maxSearchDist ) * params.maxSearchDist;
	...
	bool computeSplitDistances = false;
	{
		for ( int i = 0; i < 3; ++i )
		{
			if ( params.splitDistances[i] &&
				 params.splitDistances[i]->currentSize() == comparedCloud->size() )
			{
	...
	result = comparedOctree->executeFunctionForAllCellsAtLevel(
		params.octreeLevel,
		params.localModel == NO_MODEL ? computeCellHausdorffDistance
									  : computeCellHausdorffDistanceWithLocalModel,
		additionalParameters, params.multiThread, progressCb, "Cloud-Cloud Distance", params.maxThreadCount );
	...
	for ( unsigned i = 0; i < pointCount; i++ )
	{
												  ...
												  // point whatever its visibility is!
		{
			...
			if ( squareDist >= 0 )
			{
				...
				if ( params->CPSet )
				{
					params->CPSet->setPointIndex( cell.points->getPointGlobalIndex( i ),
				...
				if ( computeSplitDistances )
				{
					...
					if ( params->splitDistances[0] )
						params->splitDistances[0]->setValue(
							index, static_cast<ScalarType>( nNSS.queryPoint.x - P.x ) );
					if ( params->splitDistances[1] )
						params->splitDistances[1]->setValue(
							index, static_cast<ScalarType>( nNSS.queryPoint.y - P.y ) );
					if ( params->splitDistances[2] )
						params->splitDistances[2]->setValue(
							index, static_cast<ScalarType>( nNSS.queryPoint.z - P.z ) );
				}
			}
			else
			{
...
bool DistanceComputationTools::computeCellHausdorffDistanceWithLocalModel(
	const DgmOctree::octreeCell& cell, void** additionalParameters, NormalizedProgress* nProgress /*=0*/ )
{
	// additional parameters
	GenericIndexedCloudPersist* referenceCloud =
		reinterpret_cast<GenericIndexedCloudPersist*>( additionalParameters[0] );
	...
	{
		nNSS_Model.prepare( static_cast<PointCoordinateType>( params->radiusForLocalModel ),
												  ...
												  // point whatever its visibility is!
		{
			...
			if ( squareDistToNearestPoint >= 0 )
			{
				...
				if ( params->reuseExistingLocalModels )
				{
					...
					for ( std::vector<const LocalModel*>::const_iterator it = models.begin();
						  it != models.end(); ++it )
					{
				...
				if ( !lm )
				{
					...
					// actual query point!)
					{
							 ...
							 cellPos.z != nNSS_Model.cellPos.z )
						{
							...
							referenceOctree->computeCellCenter( nNSS_Model.cellPos, nNSS_Model.level,
				...
				if ( lm )
				{
					...
					ScalarType distToModel = lm->computeDistanceFromModelToPoint(
						&nNSS.queryPoint, computeSplitDistances ? &nearestModelPoint : nullptr );
					...
					if ( computeSplitDistances )
					{
						...
						if ( params->splitDistances[0] )
							params->splitDistances[0]->setValue(
								index, static_cast<ScalarType>( nNSS.queryPoint.x - nearestPoint.x ) );
						if ( params->splitDistances[1] )
							params->splitDistances[1]->setValue(
								index, static_cast<ScalarType>( nNSS.queryPoint.y - nearestPoint.y ) );
						if ( params->splitDistances[2] )
							params->splitDistances[2]->setValue(
								index, static_cast<ScalarType>( nNSS.queryPoint.z - nearestPoint.z ) );
					}
				}
				else
				{
	...
	for ( unsigned n = 0; n < numberOfTriangles; ++n )
	{
			 ...
			 GreaterThanEpsilon( CA.norm2() ) )
		{
			...
			_currentCell->level =
				octreeLevel -
				( maxSize > 1
					  ? static_cast<unsigned char>( ceil( log( static_cast<double>( maxSize ) ) / LOG_2 ) )
					  : 0 );
			...
			while ( cellsToTestCount != 0 )
			{
				...
				if ( _currentCell->level == octreeLevel )
				{
					...
					{
						if ( ( currentCellPos.x >= intersection->minFillIndexes.x &&
							   currentCellPos.x <= intersection->maxFillIndexes.x ) &&
							 ( currentCellPos.y >= intersection->minFillIndexes.y &&
							   currentCellPos.y <= intersection->maxFillIndexes.y ) &&
							 ( currentCellPos.z >= intersection->minFillIndexes.z &&
							   currentCellPos.z <= intersection->maxFillIndexes.z ) )
						{
							...
							if ( intersection->perCellTriangleList.isInitialized() )
							{
								TriangleList*& triList =
									intersection->perCellTriangleList.getValue( cellPos );
								if ( !triList )
								{
				...
				else
				{
					...
					char pointsPosition[27];
					{
						...
						for ( int i = 0; i < 3; ++i )
						{
							AB.x = distanceToOctreeMinBorder.x +
								   static_cast<PointCoordinateType>( currentCellPos.x + i * halfCellSize ) *
									   cellLength;
							for ( int j = 0; j < 3; ++j )
							{
								AB.y =
									distanceToOctreeMinBorder.y +
									static_cast<PointCoordinateType>( currentCellPos.y + j * halfCellSize ) *
										cellLength;
								for ( int k = 0; k < 3; ++k )
								{
									AB.z = distanceToOctreeMinBorder.z +
										   static_cast<PointCoordinateType>( currentCellPos.z +
																			 k * halfCellSize ) *
											   cellLength;
					...
					if ( cellsToTestCount + 27 > cellsToTest.capacity() )
					{
						...
						{
							cellsToTest.resize(
								std::max( cellsToTest.capacity() + 27, 2 * cellsToTest.capacity() ) );
						}
						catch ( const std::bad_alloc& )
						{
							// out of memory
	...
	while ( trianglesToTestCount != 0 )
	{
		...
		if ( params.signedDistances )
		{
			...
			for ( unsigned j = 0; j < remainingPoints; ++j )
			{
				...
				ScalarType dPTri = DistanceComputationTools::computePoint2TriangleDistance(
				...
				if ( !ScalarField::ValidValue( min_d ) || min_d * min_d > dPTri * dPTri )
				{
					...
					if ( params.CPSet )
					{
						...
						*const_cast<CCVector3*>( params.CPSet->getPoint( Yk.getPointGlobalIndex( j ) ) ) =
							*_nearestPoint;
					}
				}
			}
...
int ComputeMaxNeighborhoodLength( ScalarType maxSearchDist, PointCoordinateType cellSize )
{
	return static_cast<int>(
		ceil( maxSearchDist / cellSize + static_cast<ScalarType>( ( sqrt( 2.0 ) - 1.0 ) / 2 ) ) );
}
...
void cloudMeshDistCellFunc_MT( const DgmOctree::IndexAndCode& desc )
{
	...
	if ( s_params_MT.maxSearchDist > 0 )
	{
		...
		int maxNeighbourhoodLength = ComputeMaxNeighborhoodLength(
			s_params_MT.maxSearchDist, s_octree_MT->getCellSize( s_params_MT.octreeLevel ) );
		  ...
		  ++dist, maxRadius += static_cast<ScalarType>( cellLength ) )
	{
						...
						if ( triList )
						{
							if ( trianglesToTestCount + triList->indexes.size() > trianglesToTestCapacity )
							{
								trianglesToTestCapacity =
									std::max( trianglesToTestCount + triList->indexes.size(),
		...
		ComparePointsAndTriangles( Yk, remainingPoints, s_intersection_MT->mesh, trianglesToTest,
								   trianglesToTestCount, minDists, maxRadius, s_params_MT );
	}
	...
	GenericProgressCallback* progressCb /*=0*/ )
{
...
#endif
	{
		...
		if ( progressCb )
		{
			if ( progressCb->textCanBeEdited() )
			{
				...
				progressCb->setInfo( buffer );
				progressCb->setMethodTitle( params.signedDistances ? "Compute signed distances"
																   : "Compute distances" );
			}
			progressCb->update( 0 );
			progressCb->start();
		...
		for ( unsigned cellIndex = 1; cellIndex <= numberOfCells;
			  ++cellIndex, ++pCodeAndIndex ) // cellIndex = unique ID for the current cell
		{
			...
			for ( unsigned j = 0; j < remainingPoints; ++j )
			{
				...
				minDists[j] = static_cast<ScalarType>(
					DgmOctree::ComputeMinDistanceToCellBorder( *tempPt, cellLength, cellCenter ) );
			}
				  ...
				  ++dist, maxRadius += static_cast<ScalarType>( cellLength ) )
			{
				...
				ComparePointsAndTriangles( Yk, remainingPoints, mesh, trianglesToTest, trianglesToTestCount,
	...
	else
	{
		...
		{
			s_cellFunc_MT_results =
				DISTANCE_COMPUTATION_RESULTS::ERROR_EXECUTE_CLOUD_MESH_DIST_CELL_FUNC_MT_FAILURE;
		}
														 ...
														 DgmOctree* cloudOctree /*=0*/ )
{
	...
	else
	{
		...
		for ( unsigned char k = 0; k < 3; ++k )
		{
			if ( theOctreeMins.u[k] != minCubifiedBB.u[k] || theOctreeMaxs.u[k] != maxCubifiedBB.u[k] )
			{
				rebuildTheOctree = true;
				break;
			}
	...
	Tuple3ui gridSize;
	{
		for ( unsigned char k = 0; k < 3; ++k )
		{
			intersection.minFillIndexes.u[k] =
				static_cast<int>( floor( ( minBB.u[k] - minCubifiedBB.u[k] ) / cellSize ) );
			intersection.maxFillIndexes.u[k] =
				static_cast<int>( floor( ( maxBB.u[k] - minCubifiedBB.u[k] ) / cellSize ) );
			gridSize.u[k] = static_cast<unsigned>( intersection.maxFillIndexes.u[k] -
	...
	if ( result == DISTANCE_COMPUTATION_RESULTS::SUCCESS && !params.signedDistances &&
																	...
																	CCVector3* nearestP /*=0*/ )
{
	...
	CCVector3d AP( static_cast<double>( P->x ) - A->x, static_cast<double>( P->y ) - A->y,
				   static_cast<double>( P->z ) - A->z );
	CCVector3d AB( static_cast<double>( B->x ) - A->x, static_cast<double>( B->y ) - A->y,
				   static_cast<double>( B->z ) - A->z );
	CCVector3d AC( static_cast<double>( C->x ) - A->x, static_cast<double>( C->y ) - A->y,
																 ...
																 const PointCoordinateType* planeEquation )
{
	...
	return static_cast<ScalarType>(
		( CCVector3::vdot( P->u, planeEquation ) -
		  planeEquation[3] ) /*/CCVector3::vnorm(planeEquation)*/ ); // norm == 1.0!
}

ScalarType DistanceComputationTools::computePoint2LineSegmentDistSquared( const CCVector3* p,
			...
			if ( yy >= cylinderRadius2 )
			{
				if ( !solutionType )
				{
					...
					d = sqrt(
						...
						( ( x - h ) * ( x - h ) ) ); // beyond the bounds of the cylinder's axis and radius
				}
				else
				{
					d = 3;
														...
														double* rms /*= nullptr*/ )
{
	...
	for ( unsigned i = 0; i < count; ++i )
	{
		...
		CCVector3 p_inBoxCoords( pointCenterDifference.dot( u ), pointCenterDifference.dot( v ),
								 pointCenterDifference.dot( w ) );
		bool insideBox = false;
		if ( p_inBoxCoords.x > -hu && p_inBoxCoords.x < hu && p_inBoxCoords.y > -hv && p_inBoxCoords.y < hv &&
																  ...
																  float percent )
{
	...
	for ( unsigned i = 0; i < count; ++i )
	{
		...
		PointCoordinateType d =
			std::abs( CCVector3::vdot( P->u, planeEquation ) - planeEquation[3] ) /*/norm*/; // norm == 1.0
									...
									GenericProgressCallback* progressCb )
{
	...
	for ( unsigned i = 0; i < nA; ++i )
	{
		...
		comparedCloud->setPointScalarValue(
			i, ScalarField::ValidValue( dA ) && ScalarField::ValidValue( dB ) ? dA - dB : NAN_VALUE );
	}
	...
	DgmOctree* refOctree /*=0*/ )
{
	...
	if ( synchronizeOctrees( comparedCloud, referenceCloud, octreeA, octreeB, maxSearchDist, progressCb ) !=
		 SYNCHRONIZED )
	{
		return DISTANCE_COMPUTATION_RESULTS::ERROR_SYNCHRONIZE_OCTREES_FAILURE;
	}
