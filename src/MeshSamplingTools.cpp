	...
	for ( unsigned n = 0; n < mesh->size(); ++n )
	{
		...
		double signedVol =
			( -static_cast<double>( C.x * B.y * A.z ) + static_cast<double>( B.x * C.y * A.z ) +
			  ...
			  static_cast<double>( B.x * A.y * C.z ) + static_cast<double>( A.x * B.y * C.z ) ) /
			6;
...
bool MeshSamplingTools::computeMeshEdgesConnectivity( GenericIndexedMesh* mesh, EdgeConnectivityStats& stats )
{
	...
	for ( std::map<unsigned long long, unsigned>::const_iterator edgeIt = edgeCounters.begin();
												   ...
												   std::vector<unsigned>* triIndices /*=0*/ )
{
	...
	if ( sampledCloud ) // can be in case of memory overflow!
	{
		...
		{
			sampledCloud->resize(
				addedPoints ); // should always be ok as addedPoints < theoreticNumberOfPoints
			if ( triIndices )
