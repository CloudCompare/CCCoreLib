		...
		for ( unsigned i = 0; i < n; ++i )
		{
			...
			if ( ScalarField::ValidValue( V ) )
			{
				int bin = static_cast<int>(
					floor( ( V - minV ) * static_cast<ScalarType>( numberOfClasses ) / dV ) );
				if ( bin < 0 )
				{
					histoBefore++;
				}
															   ...
															   DgmOctree* inputOctree /*=0*/ )
{
	...
	unsigned numberOfChi2Classes =
		static_cast<unsigned>( ceil( sqrt( static_cast<double>( numberOfNeighbours ) ) ) );
	...
	if ( theOctree->executeFunctionForAllCellsStartingAtLevel( level, &computeLocalChi2DistAtLevel,
															   additionalParameters, numberOfNeighbours / 2,
															   numberOfNeighbours * 3, true, progressCb,
	...
	for ( unsigned i = 0; i < n; ++i )
	{
		...
		if ( ScalarField::ValidValue( D ) )
		{
			...
			double Chi2Dist = static_cast<ScalarType>( computeAdaptativeChi2Dist(
				statModel, &neighboursCloud, numberOfChi2Classes, finalNumberOfChi2Classes, true, histoMin,
