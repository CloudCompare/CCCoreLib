...
bool KDTree::findPointBelowDistance( const PointCoordinateType* queryPoint, ScalarType maxDist )
{
	if ( m_root == nullptr )
		return false;
	...
	if ( ( min <= distance + tolerance ) && ( max >= distance - tolerance ) )
	{
		if ( ( cell->leSon != nullptr ) && ( cell->gSon != nullptr ) )
		{
			...
			else
			{
				...
				{
					const CCVector3* p =
						m_associatedCloud->getPoint( m_indexes[i + cell->startingPointIndex] );
