...
//! Cholesky matrix decomposition to lower triangular matrix and its conjugate
//! transpose
...
public:
   ...
   **/
  CholeskyDecomposition(const Kriging::Matrix &matrix) : m_matrix(matrix) {
  ...
  Kriging::Vector solve(const Kriging::Vector &b) {
    ...
    // Now solve upper triangular (transpose of lower triangular) * x = y with
    // back substitution. Note that x can be solved in place using the existing
    // y vector.  No need to allocate another vector.
...
//! Lower/upper decomposition of matrix into a lower triangular matrix and a
...
public:
  ...
  /** \warning Input matrix is decomposed in-place
   **/
  LUDecomposition(Kriging::Matrix &matrix) : m_matrix(matrix) {
    assert(matrix.size() > 0 && matrix.size() == matrix[0].size());
  }
  ...
  bool decompose() {
    ...
    for (size_t p = 1; p <= n - 1; ++p) {
      ...
      for (size_t i = p + 1; i <= n; ++i) {
        if (std::abs(m_matrix[m_rowPermutation[i - 1]][p - 1]) >
            std::abs(m_matrix[m_rowPermutation[p - 1]][p - 1])) {
      ...
      // Form multiplier
      for (size_t i = p + 1; i <= n; ++i) {
        m_matrix[m_rowPermutation[i - 1]][p - 1] /=
            m_matrix[m_rowPermutation[p - 1]][p - 1];

        // Eliminate [p-1]
        for (size_t j = p + 1; j <= n; ++j) {
          m_matrix[m_rowPermutation[i - 1]][j - 1] -=
              m_matrix[m_rowPermutation[i - 1]][p - 1] *
              m_matrix[m_rowPermutation[p - 1]][j - 1];
        }
      }
    }
  ...
  //! Solve Ax = b. A is the input matrix.
  Kriging::Vector solve(const Kriging::Vector &b) {
    // The decomposed matrix is comprised of both the lower and upper diagonal
    // matrices.

    // The rows of this matrix have been permutated during the decomposition
    // process.  The m_rowPermutation indicates the proper row order.

    // The lower diagonal matrix only includes elements below the diagonal with
    // diagonal elements set to 1.

    // The upper diagonal matrix is fully specified.

    // First solve Ly = Pb for y using forward substitution. P is a permutated
    // identity matrix.
    ...
    // Note that x can be solved in place using the existing y vector. No need
...
Kriging::Kriging(const std::vector<DataPoint> &dataPoints,
                 const RasterParameters &rasterParams)
    : m_rasterParams(rasterParams), m_dataPoints(dataPoints) {}
...
struct OrdinaryKrigeContext {
  ...
  using KdTreeType = nanoflann::KDTreeSingleIndexAdaptor<
      nanoflann::L2_Simple_Adaptor<double, NFWrapper>, NFWrapper, /*dim=*/2>;

  OrdinaryKrigeContext(const std::vector<DataPoint> &_dataPoints)
      : nfWrapper(_dataPoints), kdTree(nullptr), knn(0) {}
...
bool Kriging::ordinaryKrige(const KrigeParams &params, unsigned knn,
  ...
  try {
    output.clear();
    output.reserve(static_cast<size_t>(m_rasterParams.width) *
                   m_rasterParams.height);
...
double Kriging::ordinaryKrigeSingleCell(const KrigeParams &params, unsigned row,
                                        unsigned col,
...
double Kriging::ordinaryKrigeForPoint(
    const CCVector2d &point, const KrigeParams &params,
    const std::vector<DataPoint> &dataPointCandidates) {
  // Find distances between all points and calculate covariograms
  Matrix distanceCovariogramMatrix =
      calculateCovariogramMatrix(dataPointCandidates, params, true);

  // decompose the covariogram matrix.  Because of the Lagrange multiplier, this
  // will not be a positive definite matrix and we will need to use LU
  // decomposition.
  ...
  Vector distanceCovariogramVector =
      calculateCovariogramVector(dataPointCandidates, point, params, true);

  // Solve Ax = b, for x.  A represents the covariogram matrix of all distances
  // and b is the covariogram vector for the current point. x will be a vector
  // of weights.
  ...
  return std::inner_product(
      weights.begin(), weights.end() - 1, dataPointCandidates.begin(), 0.0,
      std::plus<double>(),
      [](double weight, const DataPoint &dataPointCandidates) {
        return weight * dataPointCandidates.value;
      });
}

Kriging::Vector Kriging::calculateCovariogramVector(
    const std::vector<DataPoint> &dataPointCandidates, const CCVector2d &point,
    const KrigeParams &params, bool lagrangeMultiplier) const {
  Vector distanceVector(lagrangeMultiplier ? dataPointCandidates.size() + 1
                                           : dataPointCandidates.size(),
                        lagrangeMultiplier ? 1.0 : 0.0);
...
Kriging::Matrix Kriging::calculateCovariogramMatrix(
    const std::vector<DataPoint> &dataPointCandidates,
    const KrigeParams &params, bool lagrangeMultiplier) const {
  Matrix distanceMatrix(lagrangeMultiplier ? dataPointCandidates.size() + 1
                                           : dataPointCandidates.size(),
                        Vector(lagrangeMultiplier
                                   ? dataPointCandidates.size() + 1
                                   : dataPointCandidates.size(),
                               lagrangeMultiplier ? 1.0 : 0.0));
  ...
  for (size_t i = 0; i < dataPointCandidates.size(); ++i) {
    ...
    for (size_t j = i + 1; j < dataPointCandidates.size(); ++j) {
      double distance =
          (dataPointCandidates[i] - dataPointCandidates[j]).norm();
  ...
  if (lagrangeMultiplier) {
    distanceMatrix[dataPointCandidates.size()][dataPointCandidates.size()] =
        0.0;
  }
...
std::pair<double, double> Kriging::linearRegression(const Vector &X,
...
Kriging::KrigeParams Kriging::computeDefaultParameters() const {
  ...
  estimatedParams.sill = 0.0;
  {
    ...
    estimatedParams.sill =
        ((sum2 / m_dataPoints.size()) - (mean * mean)) / 2; // semi-variance
  }
  ...
  // estimatedParams.sill = 0.0;
  if (pointCount <= MaxPointCount) // expensive version with an explicit
                                   // computation of all possible distances!
  {
    for (size_t i = 0; i < pointCount; ++i) {
      for (size_t j = i + 1; j < pointCount; ++j) {
        ...
        if (distance >
            std::numeric_limits<double>::epsilon()) // ignore duplicate points!
        {
    ...
    // Rather than take the simple variance of the variable, a better estimation
    // should be the average variance amoung all distances.
    // estimatedParams.sill = (estimatedParams.sill / 2.0) / ((pointCount *
    // (pointCount - 1)) / 2.0); estimatedParams.sill = estimatedParams.sill /
    // (pointCount * (pointCount - 1));
  } else {
    std::mt19937 gen; // Standard mersenne_twister_engine seeded with default
                      // seed as we wan't a reproducible behavior
    ...
    // Rather than take the simple variance of the variable, a better estimation
    // should be the average variance amoung all distances.
    // estimatedParams.sill = (estimatedParams.sill / 2.0) /
    // ((semiVariogram.size() * (semiVariogram.size() - 1)) / 2.0);
    // estimatedParams.sill = estimatedParams.sill / (semiVariogram.size() *
    // (semiVariogram.size() - 1));
  }

  if (duplicatePoints) {
    // Log::Warning("Kriging", QObject::tr("Duplicate points found. Results may
    // be biased"));
  }
  ...
  double lag = 0.0, lagBoundary = 0.0;
  {
    ...
    lag = std::max(minDistance * 2.5,
                   bbDiag / 1000.0); // we can't have a too small lag value!
  ...
  for (std::multimap<double, double>::const_iterator iterator =
           semiVariogram.begin();
       iterator != semiVariogram.end() && currentLagDistance <= lagBoundary;
       ++iterator) {
    ...
    for (size_t i = 0; i < lagSemivariance.size(); ++i) {
      ...
      if (semivariance > estimatedParams.sill) {
        estimatedParams.range =
            (i != 0 ? (lagDistance[i - 1] + lagDistance[i]) / 2.0
                    : lagDistance[i]);
        peakVarianceIndex = i;
...
double Kriging::calculateCovariogram(const KrigeParams &params,
                                     double distance) const {
  // Note: as linear models do not have a sill, it is not possible to calculate
  // a covariogram.
  switch (params.model) {
  case Spherical:
    if (distance == 0.0) {
