														  ...
														  unsigned indexShift )
{
	...
	SimpleMesh* newMesh = nullptr;
	{
		...
		for ( unsigned i = 0; i < numberOfTriangles; ++i )
		{
			...
			{
				if ( count == newMesh->capacity() &&
					 !newMesh->reserve( newMesh->size() + 4096 ) ) // auto expand mesh size
				{
					// stop process
					delete newMesh;
					newMesh = nullptr;
				...
				newMesh->addTriangle( indexShift + newVertexIndexes[0], indexShift + newVertexIndexes[1],
...
const unsigned c_realIndexMask =
	0x3FFFFFFF; // original index mask (bit 0 to 29) --> max allowed index = 1073741823 ;)
const unsigned c_defaultArrayGrowth = 1024;

struct InsideOutsideIndexes
{
	InsideOutsideIndexes()
		: insideIndex( 0 )
		, outsideIndex( 0 )
	{
	}
...
bool AddVertex( CCVector3d& P, PointCloud* vertices, unsigned& index )
{
	...
	if ( vertCount == vertices->capacity() && !vertices->reserve( vertCount + c_defaultArrayGrowth ) )
	{
		// not enough memory!
		return false;
	}
...
bool AddTriangle( unsigned iA, unsigned iB, unsigned iC, SimpleMesh* mesh, bool directOrder )
{
	...
	if ( mesh->size() == mesh->capacity() && ( mesh->size() + c_defaultArrayGrowth > c_realIndexMask ||
...
{
	if ( !mesh || !vertices || mesh->size() == 0 || vertices->size() < 3 || ioParams.planeOrthoDim > 2 )
	{
	...
	try
	{
		...
		for ( unsigned i = 0; i < triCount; ++i )
		{
			...
			const unsigned origVertIndexes[3] = { tsi->i1 | c_origIndexFlag, tsi->i2 | c_origIndexFlag,
			...
			case 2: // 1 vertex 'in' the plane
			{
				...
				else
				{
					...
					unsigned iCoutside;
					unsigned iCinside;
					if ( !ComputeEdgePoint( V[iMinus], origVertIndexes[iMinus], V[iPlus],
											origVertIndexes[iPlus], iCoutside, iCinside, planeZ, Z,
					...
					if ( !AddTriangle( origVertIndexes[iCenter], origVertIndexes[iMinus], iCinside, minusMesh,
									   ( ( iCenter + 1 ) % 3 ) == iMinus )

						 || !AddTriangle( origVertIndexes[iCenter], origVertIndexes[iPlus], iCoutside,
										  plusMesh, ( ( iCenter + 1 ) % 3 ) == iPlus ) )
					{
						// early stop
						i = triCount;
						error = true;
			...
			case 3: // no vertex 'in' the plane
			{
				...
				else
				{
					...
					unsigned i2outside;
					unsigned i2inside;
					if ( !ComputeEdgePoint( V[iRight1], origVertIndexes[iRight1], V[iLeft],
											origVertIndexes[iLeft], i1outside, i1inside, planeZ, Z,
											outsideVertices, insideVertices ) ||
						 !ComputeEdgePoint( V[iRight2], origVertIndexes[iRight2], V[iLeft],
											origVertIndexes[iLeft], i2outside, i2inside, planeZ, Z,
											outsideVertices, insideVertices ) )
					{
						// early stop
						i = triCount;
					...
					if ( !AddTriangle( origVertIndexes[iLeft], leftIsMinus ? i1inside : i1outside,
									   leftIsMinus ? i2inside : i2outside, leftIsMinus ? minusMesh : plusMesh,
									   ( ( iLeft + 1 ) % 3 ) == iRight1 )

						 ||
						 !AddTriangle( leftIsMinus ? i1outside : i1inside, leftIsMinus ? i2outside : i2inside,
									   origVertIndexes[iRight1], leftIsMinus ? plusMesh : minusMesh,
									   ( ( iRight2 + 1 ) % 3 ) == iRight1 )

						 || !AddTriangle( origVertIndexes[iRight1], leftIsMinus ? i2outside : i2inside,
										  origVertIndexes[iRight2], leftIsMinus ? plusMesh : minusMesh,
										  ( ( iRight2 + 1 ) % 3 ) == iRight1 ) )
					{
						// early stop
						i = triCount;
...
bool ManualSegmentationTools::segmentMeshWithAABox( GenericIndexedMesh* origMesh,
													...
													GenericProgressCallback* progressCb /*=0*/ )
{
	if ( !origMesh || !origVertices || origMesh->size() == 0 || origVertices->size() < 3 ||
		 ioParams.bbMin.x >= ioParams.bbMax.x || ioParams.bbMin.y >= ioParams.bbMax.y ||
			...
			for ( unsigned i = 0; i < triCount; ++i )
			{
				...
				CCVector3d V[3] = {
					CCVector3d::fromArray(
						( ( vertIndexes[0] & c_origIndexFlag ) ? origVertices : sourceVertices )
							->getPoint( vertIndexes[0] & c_realIndexMask )
							->u ),
					CCVector3d::fromArray(
						( ( vertIndexes[1] & c_origIndexFlag ) ? origVertices : sourceVertices )
							->getPoint( vertIndexes[1] & c_realIndexMask )
							->u ),
					CCVector3d::fromArray(
						( ( vertIndexes[2] & c_origIndexFlag ) ? origVertices : sourceVertices )
							->getPoint( vertIndexes[2] & c_realIndexMask )
							->u )
				};
				...
				case 2: // 1 vertex 'in' the plane
				{
					...
					else
					{
						...
						if ( !ComputeEdgePoint( V[iInside], vertIndexes[iInside], V[iOuside],
												vertIndexes[iOuside], iCoutside, iCinside, planeCoord, Z,
												outsideVertices, insideVertices )

							 || !AddTriangle( vertIndexes[iCenter], vertIndexes[iInside],
											  keepBelow ? iCinside : iCoutside,
											  keepBelow ? insideMesh : outsideMesh,
											  ( ( iCenter + 1 ) % 3 ) == iInside )

							 || !AddTriangle( vertIndexes[iCenter], vertIndexes[iOuside],
						...
						if ( ioParams.trackOrigIndexes )
						{
							assert( triangleIsOriginal ||
									souceTriIndex < origTriIndexesMapInsideBackup.size() );
							unsigned origTriIndex = triangleIsOriginal
														? souceTriIndex
														: origTriIndexesMapInsideBackup[souceTriIndex];
				...
				case 3: // no vertex 'in' the plane
				{
					...
					else
					{
						...
						unsigned i2outside;
						unsigned i2inside;
						if ( !ComputeEdgePoint( V[iRight1], vertIndexes[iRight1], V[iLeft],
												vertIndexes[iLeft], i1outside, i1inside, planeCoord, Z,
												outsideVertices, insideVertices )

							 || !ComputeEdgePoint( V[iRight2], vertIndexes[iRight2], V[iLeft],
												   vertIndexes[iLeft], i2outside, i2inside, planeCoord, Z,
												   outsideVertices, insideVertices )

							 || !AddTriangle( vertIndexes[iLeft], leftIsInside ? i1inside : i1outside,
											  leftIsInside ? i2inside : i2outside,
											  leftIsInside ? insideMesh : outsideMesh,
											  ( ( iLeft + 1 ) % 3 ) == iRight1 )

							 || !AddTriangle( leftIsInside ? i1outside : i1inside,
											  leftIsInside ? i2outside : i2inside, vertIndexes[iRight1],
											  leftIsInside ? outsideMesh : insideMesh,
											  ( ( iRight2 + 1 ) % 3 ) == iRight1 )

							 || !AddTriangle( vertIndexes[iRight1], leftIsInside ? i2outside : i2inside,
											  vertIndexes[iRight2], leftIsInside ? outsideMesh : insideMesh,
											  ( ( iRight2 + 1 ) % 3 ) == iRight1 ) )
						{
							// early stop
							i = triCount;
				...
				if ( isFullyInside || isFullyOutside )
				{
					...
					else
					{
						...
						if ( !AddTriangle( vertIndexes[0], vertIndexes[1], vertIndexes[2],
										   isFullyInside ? insideMesh : outsideMesh, true ) )
						{
							// early stop
							error = true;
							break;
	...
	{
		// import the 'preserved' (original) triangles
		if ( !MergeOldTriangles( origMesh, origVertices, insideMesh, insideVertices,
								 *preservedTrianglesInside,
								 ioParams.trackOrigIndexes ? &ioParams.origTriIndexesMapInside : nullptr ) ||
			 ( ioParams.generateOutsideMesh &&
			   !MergeOldTriangles(
				   origMesh, origVertices, outsideMesh, outsideVertices, preservedTrianglesOutside,
