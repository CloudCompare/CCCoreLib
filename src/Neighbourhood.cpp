...
bool Neighbourhood::computeQuadric()
{
	...
	// conjugate gradient iterations
	{
		const double convergenceThreshold =
			lmax2 * 1.0e-8; // max. error for convergence = 1e-8 of largest cloud dimension (empirical!)
...
ScalarType Neighbourhood::computeMomentOrder1( const CCVector3& P )
{
	...
	return ( m2 < std::numeric_limits<double>::epsilon() ? NAN_VALUE
	...
	case MEAN_CURV:
	{
		...
		case MEAN_CURV:
		{
			...
			const PointCoordinateType H2 =
				std::abs( ( ( 1 + fx2 ) * fyy - 2 * fx * fy * fxy + ( 1 + fy2 ) * fxx ) ) /
				( 2 * sqrt( q ) * q );
