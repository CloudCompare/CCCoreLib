...
{
	assert( cellPos.x >= 0 && cellPos.x < MonoDimensionalCellCodes::VALUE_COUNT && cellPos.y >= 0 &&
			cellPos.y < MonoDimensionalCellCodes::VALUE_COUNT && cellPos.z >= 0 &&
			cellPos.z < MonoDimensionalCellCodes::VALUE_COUNT );
...
int DgmOctree::genericBuild( GenericProgressCallback* progressCb )
{
	...
	try
	{
		m_thePointsAndTheirCellCodes.resize(
			pointCount ); // resize + operator[] is faster than reserve + push_back!
	}
	...
	NormalizedProgress nprogress( progressCb, pointCount,
	...
	for ( unsigned i = 0; i < pointCount; i++ )
	{
		...
		if ( ( P->x >= m_pointsMin[0] ) && ( P->x <= m_pointsMax[0] ) && ( P->y >= m_pointsMin[1] ) &&
			 ( P->y <= m_pointsMax[1] ) && ( P->z >= m_pointsMin[2] ) && ( P->z <= m_pointsMax[2] ) )
		{
	...
	ParallelSort( m_thePointsAndTheirCellCodes.begin(), m_thePointsAndTheirCellCodes.end(),
				  IndexAndCode::codeComp );
	...
	if ( progressCb )
	{
		if ( progressCb->textCanBeEdited() )
		{
			...
			if ( m_numberOfProjectedPoints == pointCount )
			{
				sprintf( buffer, "[Octree::build] Octree successfully built... %u points (ok)!",
						 m_numberOfProjectedPoints );
			}
			else
			{
				if ( m_numberOfProjectedPoints == 0 )
					...
					sprintf( buffer, "[Octree::build] Warning: some points have been filtered out (%u/%u)",
							 pointCount - m_numberOfProjectedPoints, pointCount );
			}
			progressCb->setInfo( buffer );
		}

		// DGM: the dialog may close itself once we set the progress to 100% (hiding the above
		// information!)
	...
	m_nearestPow2 =
		( 1 << static_cast<int>( log( static_cast<double>( m_numberOfProjectedPoints - 1 ) ) / LOG_NAT_2 ) );
...
void DgmOctree::computeCellsStatistics( unsigned char level )
{
	...
	m_stdDevCellPopulation[level] = sqrt( sum2 / static_cast<double>( counter ) -
...
unsigned DgmOctree::getCellIndex( CellCode truncatedCellCode, unsigned char bitDec ) const
{
	...
	// http://eigenjoy.com/2011/01/21/worlds-fastest-binary-search/) DGM:	it's not faster, but the
	// code is simpler ;)
	unsigned i = 0;
	...
	for ( ; b; b >>= 1 )
	{
		...
		if ( j < m_numberOfProjectedPoints )
		{
			...
			else if ( middleCode == truncatedCellCode )
			{
				...
				if ( j == 0 ||
					 ( m_thePointsAndTheirCellCodes[j - 1].theCode >> bitDec ) != truncatedCellCode )
				{
	...
	return ( m_thePointsAndTheirCellCodes[i].theCode >> bitDec ) == truncatedCellCode
			   ? i
			   : m_numberOfProjectedPoints;
}
								  ...
								  unsigned end ) const
{
	...
	while ( true )
	{
		float centralPoint = 0.5f + 0.75f * ( static_cast<float>( truncatedCellCode - beginCode ) /
	...
	for ( ; b; b >>= 1 )
	{
		...
		if ( j < count )
		{
			...
			else if ( middleCode == truncatedCellCode )
			{
				// we must check that it's the first element equal to input code
				if ( j == 0 ||
					 ( m_thePointsAndTheirCellCodes[begin + j - 1].theCode >> bitDec ) != truncatedCellCode )
				{
					// what we are looking for is right here
					return j + begin;
				}
										...
										int neighbourhoodLength, unsigned char level ) const
{
	...
	{
		bool iBorder = ( abs( i ) == neighbourhoodLength ); // test: are we on a plane of equation
		...
		for ( int j = -jMin; j <= jMax; j++ )
		{
			...
			if ( iBorder ||
				 ( abs( j ) == neighbourhoodLength ) ) // test: are we already on one of the X or Y borders?
			{
			...
			{
				if ( kMin == neighbourhoodLength ) // test: does the plane of equation 'Z =
												   // -neighbourhoodLength' is inside the octree box?
				{
				...
				if ( kMax == neighbourhoodLength ) // test: does the plane of equation 'Z =
												   // +neighbourhoodLength' is inside the octree box?
				{
	...
	for ( int i = -iMin; i <= iMax; i++ )
	{
		...
		for ( int j = -jMin; j <= jMax; j++ )
		{
					...
					if ( index < m_numberOfProjectedPoints )
					{
						...
						try
						{
							nNSS.pointsInNeighbourhood.reserve(
								...
								static_cast<unsigned>( ceil( m_averageCellPopulation[nNSS.level] ) ) );
						}
						catch ( ... /*const std::bad_alloc&*/ ) // out of memory
						{
							// DGM TODO: Shall we stop? shall we try to go on, as we are not sure
							// that we will actually need this much points?
							assert( false );
						}
						for ( cellsContainer::const_iterator p = m_thePointsAndTheirCellCodes.begin() + index;
							  ...
							  ++p )
						{
							if ( !getOnlyPointsWithValidScalar ||
								 ScalarField::ValidValue(
									 m_theAssociatedCloud->getPointScalarValue( p->theIndex ) ) )
							{
								nNSS.pointsInNeighbourhood.emplace_back(
									m_theAssociatedCloud->getPointPersistentPtr( p->theIndex ), p->theIndex );
							}
						}
					}
				}
			...
			else // otherwise we are inside the neighbourhood
			{
				...
				{
					CellCode c2 =
						c1 | ( GenerateCellCodeForDim( nNSS.cellPos.z - neighbourhoodLength ) << 2 );
				...
				if ( kMax == neighbourhoodLength ) // test: does the plane of equation 'Z =
												   // +neighbourhoodLength' is inside the octree box?
												   // (note that neighbourhoodLength > 0)
				{
					CellCode c2 =
						c1 | ( GenerateCellCodeForDim( nNSS.cellPos.z + neighbourhoodLength ) << 2 );
												 ...
												 int maxNeighbourhoodLength ) const
{
	...
	if ( minNeighbourhoodLength == 0 ) // special case
	{
		...
		if ( index < m_numberOfProjectedPoints )
		{
			...
			for ( cellsContainer::const_iterator p = m_thePointsAndTheirCellCodes.begin() + index;
				  ...
				  ++p )
			{
				PointDescriptor newPoint( m_theAssociatedCloud->getPointPersistentPtr( p->theIndex ),
										  p->theIndex );
	...
	// get distance form cell to octree neighbourhood borders
	int limits[6];
	if ( !getCellDistanceFromBorders( nNSS.cellPos, nNSS.level, maxNeighbourhoodLength, limits ) )
		return;
		...
		for ( int v0 = nNSS.cellPos.x - iMinAbs; v0 <= nNSS.cellPos.x - minNeighbourhoodLength; ++v0 )
		{
			...
			for ( int v1 = nNSS.cellPos.y - jMinAbs; v1 <= nNSS.cellPos.y + jMaxAbs; ++v1 )
			{
				...
				for ( int v2 = nNSS.cellPos.z - kMinAbs; v2 <= nNSS.cellPos.z + kMaxAbs; ++v2 )
				{
					...
					unsigned index =
						( old_c2 < c2 ? getCellIndex( c2, bitDec, old_index, m_numberOfProjectedPoints - 1 )
		...
		for ( int v2 = nNSS.cellPos.z - kMinAbs; v2 <= nNSS.cellPos.z - minNeighbourhoodLength; ++v2 )
		{
			...
			for ( int v0 = nNSS.cellPos.x - iMinAbs; v0 <= nNSS.cellPos.x + iMaxAbs; ++v0 )
			{
				...
				for ( int v1 = nNSS.cellPos.y - jMinAbs; v1 <= nNSS.cellPos.y + jMaxAbs; ++v1 )
				{
					...
					// look for corresponding cell
					unsigned index =
						( old_c2 < c1 ? getCellIndex( c1, bitDec, old_index, m_numberOfProjectedPoints - 1 )
...
double DgmOctree::findTheNearestNeighborStartingFromCell( NearestNeighboursSearchStruct& nNSS ) const
{
	...
	if ( visitedCellDistance == 0 )
	{
		...
		CellCode truncatedCellCode = GenerateTruncatedCellCode( nNSS.cellPos, nNSS.level );
		unsigned index =
			( truncatedCellCode == INVALID_CELL_CODE ? m_numberOfProjectedPoints
	...
	PointCoordinateType minDistToBorder =
		ComputeMinDistanceToCellBorder( nNSS.queryPoint, cs, nNSS.cellCenter );
	...
	while ( true )
	{
		...
		if ( minSquareDist > 0 )
		{
			...
			int newEligibleCellDistance = static_cast<int>( ceil(
				( static_cast<PointCoordinateType>( sqrt( minSquareDist ) ) - minDistToBorder ) / cs ) );
		...
		while ( nNSS.alreadyVisitedNeighbourhoodSize <
				eligibleCellDistance ) // DGM: warning, alreadyVisitedNeighbourhoodSize == 1 means
									   // that we have only visited the first cell (distance=0)
		{
			getNeighborCellsAround( nNSS.cellPos, nNSS.minimalCellsSetToVisit,
									nNSS.alreadyVisitedNeighbourhoodSize, nNSS.level );
			  ...
			  q != nNSS.minimalCellsSetToVisit.end(); ++q )
		{
			...
			while ( m < m_numberOfProjectedPoints && ( p->theCode >> bitDec ) == code )
			{
				...
				double dist2 =
					( *m_theAssociatedCloud->getPointPersistentPtr( p->theIndex ) - nNSS.queryPoint )
						.norm2d();
		...
		// equivalent spherical neighbourhood radius (as we are actually looking to 'square'
		// neighbourhoods, we must check that the nearest points inside such neighbourhoods are
		// indeed near enough to fall inside the biggest included sphere). Otherwise we must look
		// further.
														  ...
														  bool getOnlyPointsWithValidScalar /*=false*/ ) const
{
	...
	if ( visitedCellDistance == 0 )
	{
		// visitedCellDistance == 0 means that no cell has ever been processed! No point should be
		// inside 'pointsInNeighbourhood'
		...
		if ( index < m_numberOfProjectedPoints )
		{
			...
			{
				if ( !getOnlyPointsWithValidScalar ||
					 ScalarField::ValidValue( m_theAssociatedCloud->getPointScalarValue( p->theIndex ) ) )
				{
					nNSS.pointsInNeighbourhood.emplace_back(
						m_theAssociatedCloud->getPointPersistentPtr( p->theIndex ), p->theIndex );
					++p;
	...
	while ( eligiblePoints < nNSS.minNumberOfNeighbors )
	{
		...
		while ( visitedCellDistance <
				eligibleCellDistance ) // DGM: warning, visitedCellDistance == 1 means that we
									   // have only visited the first cell (distance=0)
		{
		...
		for ( q = nNSS.pointsInNeighbourhood.begin() + eligiblePoints; q != nNSS.pointsInNeighbourhood.end();
			  ++q, ++j )
		{
	...
	std::sort( nNSS.pointsInNeighbourhood.begin(), nNSS.pointsInNeighbourhood.begin() + eligiblePoints,
												  ...
												  unsigned char level /*=0*/ ) const
{
	...
	CCVector3 boxMin( m_dimMin[0] + cs * cornerPos.x, m_dimMin[1] + cs * cornerPos.y,
...
std::size_t DgmOctree::getPointsInBoxNeighbourhood( BoxNeighbourhood& params ) const
{
	...
	if ( params.axes )
	{
		...
		CCVector3 corners[8] = {
			CCVector3( -dx / 2, -dy / 2, -dz / 2 ), CCVector3( -dx / 2, -dy / 2, dz / 2 ),
			CCVector3( -dx / 2, dy / 2, dz / 2 ),	CCVector3( -dx / 2, dy / 2, -dz / 2 ),
			CCVector3( dx / 2, -dy / 2, -dz / 2 ),	CCVector3( dx / 2, -dy / 2, dz / 2 ),
			CCVector3( dx / 2, dy / 2, dz / 2 ),	CCVector3( dx / 2, dy / 2, -dz / 2 )
		};
		...
		{
			corners[i] =
				corners[i].x * params.axes[0] + corners[i].y * params.axes[1] + corners[i].z * params.axes[2];
			if ( i )
			{
			...
			for ( int k = minCornerPos.z; k <= maxCornerPos.z; ++k )
			{
				...
				if ( params.axes )
				{
					CCVector3 cellCenter =
						m_dimMin + CCVector3( static_cast<PointCoordinateType>( i + 0.5 ),
					...
					if ( std::abs( Q.x ) > maxHalfDist.x || std::abs( Q.y ) > maxHalfDist.y ||
				...
				if ( cellIndex < m_numberOfProjectedPoints )
				{
					// we look for the first index in 'm_thePointsAndTheirCellCodes' corresponding
					// to this cell
					cellsContainer::const_iterator p = m_thePointsAndTheirCellCodes.begin() + cellIndex;
					CellCode searchCode = ( p->theCode >> bitDec );
...
std::size_t DgmOctree::getPointsInCylindricalNeighbourhood( CylindricalNeighbourhood& params ) const
{
	...
	PointCoordinateType maxLengthFactor =
		params.maxHalfLength + static_cast<PointCoordinateType>( cs * SQRT_3 / 2 );
		...
		while ( cellMin.y < maxCorner.y && cellPos.y <= maxFillIndexes[1] )
		{
			...
			while ( cellMin.z < maxCorner.z && cellPos.z <= maxFillIndexes[2] )
			{
				...
				if ( d2 <= maxDiagFactor && dot <= maxLengthFactor &&
					 dot >= minLengthFactor ) // otherwise cell is totally outside
				{
					// 2nd test: does this cell exists?
					CellCode truncatedCellCode = GenerateTruncatedCellCode( cellPos, params.level );
					unsigned cellIndex = getCellIndex( truncatedCellCode, bitDec );
					...
					if ( cellIndex < m_numberOfProjectedPoints )
					{
							  ...
							  ++p )
						{
							...
							{
								params.neighbours.emplace_back( P, p->theIndex,
																dot ); // we save the distance relatively to
																	   // the center projected on the axis!
							}
						}
					}
				}
								...
								if ( d2 <= squareRadius )
								{
									...
									if ( dot >= currentHalfLengthMinus && dot <= params.currentHalfLength )
									{
										params.neighbours.emplace_back(
											P, p->theIndex,
											dot ); // we save the distance relatively to the
												   // center projected on the axis!
									}
									else if ( params.currentHalfLength < params.maxHalfLength )
									{
										...
										params.potentialCandidates.emplace_back(
											P, p->theIndex,
											dot ); // we save the distance relatively to the center
													   ...
													   double radius, bool sortValues ) const
{
	...
	if ( !nNSS.ready )
	{
		...
		nNSS.cellsInNeighbourhood.reserve( minNeighbourhoodSize * minNeighbourhoodSize *
										   minNeighbourhoodSize );

		if ( nNSS.alreadyVisitedNeighbourhoodSize == 1 && nNSS.cellsInNeighbourhood.empty() &&
			 !nNSS.pointsInNeighbourhood.empty() )
		{
			// in this case, we assume the points already in 'pointsInNeighbourhood' are the 1st
			// cell points
	...
	const PointCoordinateType minDistToBorder =
		ComputeMinDistanceToCellBorder( nNSS.queryPoint, cs, nNSS.cellCenter );

	// we deduce the minimum cell neighbourhood size (integer) that includes the search sphere
	const int minNeighbourhoodSize =
		1 + ( radius > minDistToBorder ? static_cast<int>( ceil( ( radius - minDistToBorder ) / cs ) ) : 0 );
	...
	// const PointCoordinateType& half_cs=getCellSize(nNSS.level+1); //half cell size at current
	// level CCVector3 limitMin =
	// nNSS.queryPoint-CCVector3(radius,radius,radius)-CCVector3(half_cs,half_cs,half_cs); CCVector3
	// limitMax = nNSS.queryPoint+CCVector3(radius,radius,radius)+CCVector3(half_cs,half_cs,half_cs);
		  ...
		  c != nNSS.cellsInNeighbourhood.end(); ++c )
	{
		...
		if ( d2 <= nNSS.minOutD2 )
		{
			...
			unsigned count = ( ( c + 1 ) != nNSS.cellsInNeighbourhood.end()
								   ? ( c + 1 )->index
	...
	if ( sortValues && numberOfEligiblePoints > 0 )
	{
		std::sort( nNSS.pointsInNeighbourhood.begin(),
				   nNSS.pointsInNeighbourhood.begin() + numberOfEligiblePoints, PointDescriptor::distComp );
	}
...
unsigned char DgmOctree::findBestLevelForAGivenNeighbourhoodSizeExtraction( PointCoordinateType radius ) const
{
	...
	PointCoordinateType aim =
		std::max<PointCoordinateType>( 0, radius / c_neighbourhoodSizeExtractionFactor );
...
unsigned char DgmOctree::findBestLevelForComparisonWithOctree( const DgmOctree* theOtherOctree ) const
{
	...
	for ( int i = 1; i < maxOctreeLevel; ++i ) // warning: i >= 1
	{
		...
		bool success = diff( i, m_thePointsAndTheirCellCodes, theOtherOctree->m_thePointsAndTheirCellCodes,
							 diffA, diffB, cellsA, cellsB );
	...
	unsigned indicativeNumberOfPointsPerCell ) const
{
	for ( unsigned char level = MAX_OCTREE_LEVEL; level > 0; --level )
	{
		if ( m_averageCellPopulation[level] >
			 indicativeNumberOfPointsPerCell ) // density can only increase. If it's above the
											   // target, no need to look further
		{
			// we take the closest match between this level and the previous one
			if ( level == MAX_OCTREE_LEVEL ||
				 ( m_averageCellPopulation[level] - indicativeNumberOfPointsPerCell <=
				   indicativeNumberOfPointsPerCell -
					   m_averageCellPopulation[level + 1] ) ) // by definition
										...
										bool truncatedCodes /*=false*/ ) const
{
	try
	{
		...
		CellCode predCode =
			( p->theCode >> bitDec ) + 1; // pred value must be different than the first element's
					  ...
					  int& diffA, int& diffB, int& cellsA, int& cellsB ) const
{
	...
	while ( ( pA != codesA.end() ) && ( pB != codesB.end() ) )
	{
		if ( predCodeA < predCodeB )
		{
			++diffA;
			++cellsA;
			while ( ( pA != codesA.end() ) && ( ( currentCodeA = ( pA->theCode >> bitDec ) ) == predCodeA ) )
				++pA;
		...
		else if ( predCodeA > predCodeB )
		{
			++diffB;
			++cellsB;
			while ( ( pB != codesB.end() ) && ( ( currentCodeB = ( pB->theCode >> bitDec ) ) == predCodeB ) )
				++pB;
					...
					if ( smallestLabel != neighboursVal.back() )
					{
						...
						lastLabel = smallestLabel;
						{
							for ( std::size_t n = 1; n < neighboursMin.size(); n++ )
							{
								...
								assert( label < static_cast<int>( numberOfCells ) + 2 );
								// we don't process it if it's the same label as the previous
								// neighbor
								if ( label != lastLabel )
								{
...
#endif
	{
		...
		if ( progressCb )
		{
			if ( progressCb->textCanBeEdited() )
			{
				...
				sprintf( buffer,
						 ...
						 "population: %u",
						 level, cellCount, m_averageCellPopulation[level], m_stdDevCellPopulation[level],
		...
		if ( fp )
		{
			...
			fprintf( fp, "Tested:   %f (%3.1f %%)\n", s_testedPoints,
					 100.0 * s_testedPoints / std::max( 1.0, s_testedPoints + s_skippedPoints ) );
			fprintf( fp, "skipped: %f (%3.1f %%)\n", s_skippedPoints,
					 100.0 * s_skippedPoints / std::max( 1.0, s_testedPoints + s_skippedPoints ) );
		...
		if ( progressCb )
		{
			if ( progressCb->textCanBeEdited() )
			{
				...
				sprintf( buffer,
						 ...
						 "population: %u",
						 level, static_cast<int>( cells.size() ), m_averageCellPopulation[level],
						 m_stdDevCellPopulation[level], m_maxCellPopulation[level] );
				progressCb->setInfo( buffer );
			}
			progressCb->update( 0 );
	...
	GenericProgressCallback* progressCb /*=0*/, const char* functionTitle /*=0*/, int maxThreadCount /*=0*/ )
{
...
#endif
	{
		...
		if ( progressCb )
		{
			if ( progressCb->textCanBeEdited() )
			{
				...
				sprintf( buffer,
						 ...
						 "(+/-%3.2f) - %3.2f (+/-%3.2f)\nMax population: %u - %u",
						 startingLevel, MAX_OCTREE_LEVEL, getCellNumber( startingLevel ),
						 getCellNumber( MAX_OCTREE_LEVEL ), m_averageCellPopulation[startingLevel],
						 m_stdDevCellPopulation[startingLevel], m_averageCellPopulation[MAX_OCTREE_LEVEL],
						 m_stdDevCellPopulation[MAX_OCTREE_LEVEL], m_maxCellPopulation[startingLevel],
		...
		while ( cell.index < m_numberOfProjectedPoints )
		{
			...
			// we can already 'add' (virtually) the first point to the current cell description
			// struct
			unsigned elements = 1;
			...
			if ( progressCb )
			{
				progressCb->update( 100.0f * static_cast<float>( cell.index ) /
				...
				if ( currentTruncatedCode == cell.truncatedCode )
				{
					...
					if ( elements == maxNumberOfPointsPerCell )
					{
						...
						while ( cell.level < MAX_OCTREE_LEVEL )
						{
							...
							{
								// we must re-check all the previous inserted points at this new
								// level to determine the end of this new cell
								p = startingElement;
								elements = 1;
				...
				if ( currentTruncatedCode == cellDesc.truncatedCode )
				{
					...
					if ( elements == maxNumberOfPointsPerCell )
					{
						...
						while ( cellDesc.level < MAX_OCTREE_LEVEL )
						{
							...
							{
								// we must re-check all the previously inserted points at this new
								// level to determine the end of this new cell
								p = startingElement;
								elements = 1;
		...
		if ( progressCb )
		{
			if ( progressCb->textCanBeEdited() )
			{
				...
				sprintf( buffer,
						 "Octree levels %i - %i\nCells: %i\nAverage population: %3.2f "
						 "(+/-%3.2f)\nMax population: %llu",
						 startingLevel, MAX_OCTREE_LEVEL, static_cast<int>( cells.size() ), mean, stddev,
						 maxPop );
				progressCb->setInfo( buffer );
			}
			if ( s_normProgressCb_MT )
						 ...
						 bool isFOV, RayCastProcess process, std::vector<PointDescriptor>& output ) const
{
		  ...
		  it != m_thePointsAndTheirCellCodes.end(); ++it )
	{
		...
		{
			// look for the biggest 'parent' cell that englobes this cell and the previous one (if
			// any)
			while ( level > 1 )
			{
