	...
	GenericProgressCallback* progressCb /*=nullptr*/, DgmOctree* inputOctree /*=nullptr*/ )
{
	...
	// parameters
	void* additionalParameters[] = { static_cast<void*>( &c ), static_cast<void*>( &subOption ),
									 static_cast<void*>( &kernelRadius ) };

	ErrorCode result = NoError;

	if ( octree->executeFunctionForAllCellsAtLevel( level, &ComputeGeomCharacteristicAtLevel,
	...
	for ( unsigned i = 0; i < n; ++i )
	{
		...
		case Feature:
			if ( neighborCount > 3 )
			{
				...
				value = static_cast<ScalarType>(
					Z.computeFeature( static_cast<Neighbourhood::GeomFeature>( subOption ) ) );
			}
			break;
		...
		case Curvature:
			if ( neighborCount > 5 )
			{
				DgmOctreeReferenceCloud neighboursCloud( &nNSS.pointsInNeighbourhood, neighborCount );
				Neighbourhood Z( &neighboursCloud );
				value = Z.computeCurvature( nNSS.queryPoint,
											static_cast<Neighbourhood::CurvatureType>( subOption ) );
			}
			break;
		...
		case Roughness:
			if ( neighborCount > 3 )
			{
				...
				DgmOctreeReferenceCloud neighboursCloud(
					...
					neighborCount - 1 ); // we don't take the query point into account!
				Neighbourhood Z( &neighboursCloud );
	...
	GenericProgressCallback* progressCb /*=0*/, DgmOctree* inputOctree /*=0*/ )
{
	...
	unsigned char level = theOctree->findBestLevelForAGivenNeighbourhoodSizeExtraction(
	...
	if ( theOctree->executeFunctionForAllCellsAtLevel( level, &FlagDuplicatePointsInACellAtLevel,
				...
				for ( unsigned j = 0; j < neighborCount; ++j )
				{
					if ( nNSS.pointsInNeighbourhood[j].pointIndex != iIndex )
					{
						...
						cell.points->getAssociatedCloud()->setPointScalarValue(
							nNSS.pointsInNeighbourhood[j].pointIndex, static_cast<ScalarType>( 1 ) );
					}
				}
			}
		}
	...
	DgmOctree* inputOctree /*=0*/ )
{
	...
	if ( theOctree->executeFunctionForAllCellsAtLevel( level, &ComputeApproxPointsDensityInACellAtLevel,
													   additionalParameters, true, progressCb,
...
SquareMatrixd GeometricalAnalysisTools::ComputeCovarianceMatrix( GenericCloud* cloud,
																 const PointCoordinateType* _gravityCenter )
{
	assert( cloud );
	...
	unsigned seed /*=0*/ )
{
	...
	if ( n > p )
	{
		m = static_cast<unsigned>( log( 1.0 - confidence ) /
