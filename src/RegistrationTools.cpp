		...
		if ( filters & SKIP_RYZ ) // keep only the rotation component around X
		{
			...
			if ( R.getValue( 0, 2 ) < 1.0 )
			{
				...
				PointCoordinateType theta =
					atan2( R.getValue( 1, 2 ) / cos_phi, R.getValue( 2, 2 ) / cos_phi );
		...
		else if ( filters & SKIP_RXZ ) // keep only the rotation component around Y
		{
			...
			if ( R.getValue( 2, 1 ) < 1.0 )
			{
				...
				PointCoordinateType phi =
					atan2( -R.getValue( 2, 0 ) / cos_theta, R.getValue( 2, 2 ) / cos_theta );
				PointCoordinateType cos_phi = cos( phi );
		...
		else if ( filters & SKIP_RXY ) // keep only the rotation component around Z
		{
			...
			if ( R.getValue( 2, 0 ) < 1.0 )
			{
				...
				PointCoordinateType phi_rad =
					atan2( R.getValue( 1, 0 ) / cos_theta, R.getValue( 0, 0 ) / cos_theta );
	...
	double& finalRMS, unsigned& finalPointCount, GenericProgressCallback* progressCb /*=0*/ )
{
	...
	else /*if (inputModelCloud)*/
	{
		...
		if ( inputModelCloud->size() > params.samplingLimit )
		{
			ReferenceCloud* subModelCloud =
				CloudSamplingTools::subsampleCloudRandomly( inputModelCloud, params.samplingLimit );
			if ( !subModelCloud )
			{
				// not enough memory
				return ICP_ERROR_NOT_ENOUGH_MEMORY;
	...
	// data.cloud->forEach(ScalarFieldTools::SetScalarValueToNaN); //DGM: done automatically in
	// computeCloud2CloudDistance now
	if ( inputModelMesh )
	{
		...
		if ( DistanceComputationTools::computeCloud2MeshDistance( data.cloud, inputModelMesh, c2mDistParams,
																  progressCb ) < 0 )
		{
	...
	else if ( inputModelCloud )
	{
		...
		if ( DistanceComputationTools::computeCloud2CloudDistance( data.cloud, model.cloud, c2cDistParams,
																   progressCb ) < 0 )
		{
			// an error occurred during distances computation...
			return ICP_ERROR_DIST_COMPUTATION;
	...
	for ( unsigned iteration = 0;; ++iteration )
	{
			...
			if ( N.isValid() )
			{
				...
				if ( data.CPSetRef )
				{
					filteredData.CPSetRef = new ReferenceCloud(
						data.CPSetRef->getAssociatedCloud() ); // we must also update the CPSet!
				...
				filteredData.cloud->resize(
					filteredData.cloud->size() ); // should always be ok as current size < pointCount
				if ( filteredData.CPSetRef )
		...
		if ( coupleWeights )
		{
			...
			{
				ScalarType wd =
					( data.weights ? data.weights->getValue( i ) : static_cast<ScalarType>( 1.0 ) );
				ScalarType wm =
					( model.weights ? model.weights->getValue( data.CPSetRef->getPointGlobalIndex( i ) )
									: static_cast<ScalarType>(
										  1.0 ) ); // model weights are only support with a reference cloud!
			...
			else
			{
				...
				if ( ( params.convType == MAX_ERROR_CONVERGENCE &&
					 ...
					 || ( params.convType == MAX_ITER_CONVERGENCE &&
						  iteration >= params.nbMaxIterations ) // max iteration reached
				)
				{
				...
				if ( progressCb )
				{
					...
					else
					{
						...
						float progressPercent =
							static_cast<float>( ( initialDeltaRMS - deltaRMS ) /
												( initialDeltaRMS - params.minRMSDecrease ) * 100.0 );
		...
		if ( !RegistrationTools::RegistrationProcedure( data.cloud,
														data.CPSetRef
															? static_cast<GenericCloud*>( data.CPSetRef )
															: static_cast<GenericCloud*>( data.CPSetPlain ),
		...
		if ( !data.rotatedCloud || pointOrderHasBeenChanged )
		{
			...
			PointCloud* rotatedDataCloud =
				PointProjectionTools::applyTransformation( data.cloud, currentTrans );
			if ( !rotatedDataCloud )
			{
				// not enough memory
				result = ICP_ERROR_NOT_ENOUGH_MEMORY;
		...
		if ( inputModelMesh )
		{
			...
			c2mDistParams.CPSet = data.CPSetPlain;
			c2mDistParams.maxThreadCount = params.maxThreadCount;
			if ( DistanceComputationTools::computeCloud2MeshDistance( data.cloud, inputModelMesh,
																	  c2mDistParams ) < 0 )
			{
				// an error occurred during distances computation...
				result = ICP_ERROR_REGISTRATION_STEP;
		...
		else if ( inputDataCloud )
		{
			...
			c2cDistParams.CPSet = data.CPSetRef;
			c2cDistParams.maxThreadCount = params.maxThreadCount;
			if ( DistanceComputationTools::computeCloud2CloudDistance( data.cloud, model.cloud,
																	   c2cDistParams ) < 0 )
			{
				// an error occurred during distances computation...
				result = ICP_ERROR_REGISTRATION_STEP;
											   ...
											   PointCoordinateType aPrioriScale /*=1.0f*/ )
{
	...
	// centers of mass
	CCVector3 Gp = coupleWeights ? GeometricalAnalysisTools::ComputeWeightedGravityCenter( P, coupleWeights )
								 : GeometricalAnalysisTools::ComputeGravityCenter( P );
	CCVector3 Gx = coupleWeights ? GeometricalAnalysisTools::ComputeWeightedGravityCenter( X, coupleWeights )
	...
	if ( P->size() == 3 )
	{
		...
		if ( adjustScale )
		{
			...
			trans.s =
				static_cast<PointCoordinateType>( sumNormX / sumNormP ); // sumNormX / (sumNormP * Sa) in fact
		}

		// we deduce the first translation
		trans.T =
			Gx - ( trans.R * Gp ) *
					 ( aPrioriScale * trans.s ); //#26 in besl paper, modified with the scale as in jschmidt
	...
	else
	{
		...
		SquareMatrixd Sigma_px =
			( coupleWeights ? GeometricalAnalysisTools::ComputeWeightedCrossCovarianceMatrix( P, X, Gp, Gx,
																							  coupleWeights )
		...
		if ( adjustScale )
		{
			...
			for ( unsigned i = 0; i < count; ++i )
			{
				...
				CCVector3 a_tilde =
					trans.R * ( *( P->getNextPoint() ) - Gp );		 // a_tilde_i = R * (a_i - a_mean)
											...
											unsigned nbMaxCandidates )
{
	...
	for ( unsigned i = 0; i < nbBases; i++ )
	{
		...
		// Compute rigid transforms and filter bases if necessary
		{
			...
			if ( !FilterCandidates( modelCloud, dataCloud, reference, candidates, nbMaxCandidates,
									transforms ) )
			{
				delete dataTree;
				delete modelTree;
									  ...
									  unsigned nbTries, Base& base )
{
	...
	for ( i = 0; i < nbTries; i++ )
	{
		...
		d1 = ( *p3 - *p1 ).norm2();
		d2 = ( *p3 - *p2 ).norm2();
		if ( ( d0 >= overlap && d1 >= overlap ) || ( d0 >= overlap && d2 >= overlap ) ||
											  ...
											  std::vector<ScaledTransformation>& transforms )
{
	...
	// enough memory?
	if ( scores.capacity() < candidatesCount || sortedscores.capacity() < candidatesCount ||
		 tarray.capacity() < candidatesCount || transforms.capacity() < candidatesCount )
	{
		return false;
	}
