...
float FastMarching::getTime( Tuple3i& pos, bool absoluteCoordinates ) const
{
	...
	{
		index = static_cast<unsigned>( pos.x + 1 ) + static_cast<unsigned>( pos.y + 1 ) * m_rowSize +
	...
	for ( unsigned i = 0; i < CC_FM_MAX_NUMBER_OF_NEIGHBOURS; ++i )
	{
		...
		m_neighboursDistance[i] =
			sqrt( static_cast<float>(
				c_FastMarchingNeighbourPosShift[i * 3] * c_FastMarchingNeighbourPosShift[i * 3] +
...
float FastMarching::computeT( unsigned index )
{
			...
			if ( nCell && ( nCell->state == Cell::TRIAL_CELL || nCell->state == Cell::ACTIVE_CELL ) )
			{
				...
				T[n] = static_cast<double>( nCell->T ) +
					   static_cast<double>( m_neighboursDistance[n] ) *
						   static_cast<double>( computeTCoefApprox( nCell, theCell ) );
			}
			else
			{
				// no front yet
	...
	// Quadratic eq. along X
	{
		...
		for ( unsigned n = 0; n < m_numberOfNeighbours; ++n )
			if ( c_FastMarchingNeighbourPosShift[n * 3] != 0 )
				if ( T[n] < Tmin )
					Tmin = T[n];
		if ( Tij > Tmin )
		{
	...
	// Quadratic eq. along Y
	{
		...
		double Tmin = static_cast<double>( Cell::T_INF() );
		for ( unsigned n = 0; n < m_numberOfNeighbours; ++n )
			if ( c_FastMarchingNeighbourPosShift[n * 3 + 1] != 0 )
				if ( T[n] < Tmin )
					Tmin = T[n];
		if ( Tij > Tmin )
		{
	...
	// Quadratic eq. along Z
	{
		...
		double Tmin = static_cast<double>( Cell::T_INF() );
		for ( unsigned n = 0; n < m_numberOfNeighbours; ++n )
			if ( c_FastMarchingNeighbourPosShift[n * 3 + 2] != 0 )
				if ( T[n] < Tmin )
					Tmin = T[n];
		if ( Tij > Tmin )
		{
