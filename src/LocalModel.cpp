...
public:
	...
	LOCAL_MODEL_TYPES getType() const override
	{
		return LS;
	}
												...
												CCVector3* nearestPoint = nullptr ) const override
	{
		...
		if ( m_tri )
		{
			...
			for ( unsigned i = 0; i < numberOfTriangles; ++i )
			{
				...
				ScalarType dist2 = DistanceComputationTools::computePoint2TriangleDistance(
					P, tri, false, nearestPoint ? &triNearestPoint : nullptr );
...
public:
	...
	// inherited from LocalModel
	ScalarType computeDistanceFromModelToPoint( const CCVector3* _P,
												CCVector3* nearestPoint = nullptr ) const override
	{
		CCVector3 P = *_P - m_gravityCenter;

		// height = h0 + h1.x + h2.y + h3.x^2 + h4.x.y + h5.y^2
		PointCoordinateType z = m_eq[0] + m_eq[1] * P.u[m_X] + m_eq[2] * P.u[m_Y] +
